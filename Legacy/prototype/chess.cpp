...
enum Player : uint8_t { White = 0, Black, PlayerCount };
...
template <> struct Action<Chess> {
  ...
  bool operator==(const Action<Chess> &par_rhs) const {
    return orig == par_rhs.orig && dest == par_rhs.dest &&
           promotion == par_rhs.promotion;
  }
...
template <> struct State<Chess> {
  State() {
    board = {___};
    board[A2] = board[B2] = board[C2] = board[D2] = board[E2] = board[F2] =
        board[G2] = board[H2] = W_P;
    ...
    board[A7] = board[B7] = board[C7] = board[D7] = board[E7] = board[F7] =
        board[G7] = board[H7] = B_P;
...
template <> struct PathCost<Chess> {
  PathCost() {
    piece_weight = 1.f;
    child_count = 0;
    min_child_cost = 0.f;
  ...
  PathCost(const State<Chess> &par_state) {
    ...
    piece_weight =
        player == White ? (w_pieces / b_pieces) : (b_pieces / w_pieces);
    child_count = 0;
    min_child_cost = 0.f;
    expanded = false;
  }
...
void ForEachSpaceHorizontalAndVertical(
    const std::function<void(Space)> &par_function,
    const std::array<Piece, SpaceCount> &par_board, Space par_space) {
  ...
  for (int8_t h = -1; h <= 1; h += 1) {
    for (int8_t v = -1; v <= 1; v += 1) {
      ...
      for (Coord dest = {int8_t(coord.c + h), int8_t(coord.r + v)};
           0 <= dest.c && dest.c < 8 && 0 <= dest.r && dest.r < 8;
           dest.c += h, dest.r += v) {
...
void ForEachSpaceDiagonal(const std::function<void(Space)> &par_function,
                          const std::array<Piece, SpaceCount> &par_board,
                          Space par_space) {
  const Coord coord(par_space);
...
void ForEachSpaceL(const std::function<void(Space)> &par_function,
                   Space par_space) {
  const Coord coord(par_space);
  ...
  for (int8_t c = -2; c <= 2; ++c) {
    for (int8_t r = -2; r <= 2; ++r) {
      ...
      if (coord.c + c < 0 || coord.c + c >= 8 || coord.r + r < 0 ||
          coord.r + r >= 8)
        continue;
...
void PawnActions(std::vector<Action<Chess>> &par_actions,
                 const State<Chess> &par_state, Space par_space) {
  const Coord coord(par_space);
  const Player player = Player(par_state.moves.size() % 2);
  ...
  if (board[space_forward_1] == ___) {
    if (coord.r + v == r_final) {
      par_actions.emplace_back(par_space, space_forward_1,
                               player == White ? W_N : B_N);
      par_actions.emplace_back(par_space, space_forward_1,
                               player == White ? W_B : B_B);
      par_actions.emplace_back(par_space, space_forward_1,
                               player == White ? W_R : B_R);
      par_actions.emplace_back(par_space, space_forward_1,
                               player == White ? W_Q : B_Q);
    } else {
      par_actions.emplace_back(par_space, space_forward_1);
    }
  ...
  for (int8_t h = -1; h <= 1; h += 2) {
    ...
    if (0 <= diag.c && diag.c < 8) {
      ...
      if (coord.r == r_en_pessant) {
        ...
        if ((piece_side == W_P || piece_side == B_P) &&
            ToPlayer(piece_side) != player) {
          ...
          if (last_move.dest == space_side &&
              last_move_orig.r - last_move_dest.r == 2) {
            par_actions.emplace_back(par_space, space_diag);
          }
        }
      }
...
void KnightActions(std::vector<Action<Chess>> &par_actions,
                   const State<Chess> &par_state, Space par_space) {
  const Player player = Player(par_state.moves.size() % 2);
  const auto &board = par_state.board;

  ForEachSpaceL(
      [&](Space par_space_dest) {
...
void BishopActions(std::vector<Action<Chess>> &par_actions,
                   const State<Chess> &par_state, Space par_space) {
  const Player player = Player(par_state.moves.size() % 2);
  const auto &board = par_state.board;

  ForEachSpaceDiagonal(
      [&](Space par_space_dest) {
        const Piece piece_dest = board[par_space_dest];
...
void RookActions(std::vector<Action<Chess>> &par_actions,
                 const State<Chess> &par_state, Space par_space) {
  const Coord coord(par_space);
  const Player player = Player(par_state.moves.size() % 2);
  const auto &board = par_state.board;
...
void KingActions(std::vector<Action<Chess>> &par_actions,
                 const State<Chess> &par_state, Space par_space) {
  const Coord coord(par_space);
  const Player player = Player(par_state.moves.size() % 2);
  const auto &board = par_state.board;
  ...
  if (par_state.king_moved[player] == false) {
    ...
    if (par_state.rookH_moved[player] == false) {
      const bool no_pieces_between = (player == White)
                                         ? board[F1] == ___ && board[G1] == ___
                                         : board[F8] == ___ && board[G8] == ___;

      if (no_pieces_between) {
        bool any_check = false;

        for (int i = 0; i < 3; ++i) {
          const Action<Chess> action = {par_space,
                                        Space(Coord(coord.c + i, coord.r))};
        ...
        if (any_check == false) {
          par_actions.emplace_back(par_space,
                                   Space(Coord(coord.c + 2, coord.r)));
        }
      }
    }
    ...
    if (par_state.rookA_moved[player] == false) {
      const bool no_pieces_between =
          (player == White)
              ? board[B1] == ___ && board[C1] == ___ && board[D1] == ___
              : board[B8] == ___ && board[C8] == ___ && board[D8] == ___;
...
bool IsCheckForPlayer(const std::array<Piece, SpaceCount> &par_board,
                      Player par_player, Space par_king) {
      ...
      [&](Space par_space) {
        ...
        if (piece != ___ && ToPlayer(piece) != par_player) {
          ...
          if (piece == W_K || piece == B_K) {
            ...
            if (std::abs(coord.c - coord_king.c) +
                    std::abs(coord.r - coord_king.r) ==
                1) {
              check = true;
              return;
            }
  ...
  ForEachSpaceDiagonal(
      [&](Space par_space) {
        ...
        if (piece != ___ && ToPlayer(piece) != par_player) {
          ...
          if (std::abs(coord.c - coord_piece.c) +
                  std::abs(coord.r - coord_piece.r) ==
              2) {
            if (piece == W_K || piece == B_K) {
              check = true;
              return;
            ...
            if (piece == W_P || piece == B_P) {
              if ((par_player == White && (coord_piece.r == coord.r + 1)) ||
                  (par_player == Black && (coord_piece.r == coord.r - 1))) {
                check = true;
                return;
              }
            }
...
template <>
std::vector<Action<Chess>> Problem<Chess>::Actions(State<Chess> par_state) {
...
template <>
State<Chess> Problem<Chess>::Result(State<Chess> par_state,
                                    Action<Chess> par_action) {
  ...
  // En Pessant
  if ((piece_orig == W_P || piece_orig == B_P) && piece_dest == ___ &&
      orig.c != dest.c) {
    const Space space_captured_pawn =
        Coord(dest.c, player == White ? dest.r - 1 : dest.r + 1);
  ...
  // Castle
  if ((piece_orig == W_K || piece_orig == B_K) &&
      std::abs(orig.c - dest.c) == 2) {
  ...
  // Promotion
  if ((piece_orig == W_P && dest.r == 7) ||
      (piece_orig == B_P && dest.r == 0)) {
  ...
  if (par_action.orig == A1)
    state.rookA_moved[White] = true;
  if (par_action.orig == A8)
    state.rookA_moved[Black] = true;
  if (par_action.orig == H1)
    state.rookH_moved[White] = true;
  if (par_action.orig == H8)
    state.rookH_moved[Black] = true;
...
template <>
PathCost<Chess> Problem<Chess>::C(State<Chess> par_state,
                                  Action<Chess> par_action,
                                  State<Chess> par_state_p) {
  assert(0);
  return {};
}
  ...
  Action<Chess> operator()(std::vector<Action<Chess>> par_actions) {
    for (Action<Chess> action : par_actions) {
      ...
      auto it = std::find_if(
          ...
          [&](Node<Chess> *par_node) { return par_node->action == action; });
...
private:
  ...
  Action<Chess> Result() const {
    ...
    auto it = std::ranges::min_element(
        root->children, [](Node<Chess> *par_lhs, Node<Chess> *par_rhs) {
  ...
  void Think() {
    ...
    while (node->path_cost.expanded == true) {
      node = *std::ranges::min_element(
          node->children, [](Node<Chess> *par_lhs, Node<Chess> *par_rhs) {
            return par_lhs->path_cost < par_rhs->path_cost;
          });
    }
  ...
  Node<Chess> *NodeConstruct(State<Chess> par_state,
                             Node<Chess> *par_parent = nullptr,
                             std::optional<Action<Chess>> par_action = {}) {
  ...
  void NodeExpand(Node<Chess> *par_node) {
    ...
    const std::vector<Action<Chess>> actions =
        Problem<Chess>::Actions(par_node->state);

    for (const Action<Chess> &action : actions) {
      const State<Chess> state =
          Problem<Chess>::Result(par_node->state, action);
      if (not state.check) {
  ...
  void UpdateTree(Node<Chess> *par_node) {
    ...
    while (node != nullptr) {
      ...
      if (node->children.size() > 0) {
        node->path_cost.min_child_cost =
            (*std::ranges::min_element(
                 node->children,
                 [](Node<Chess> *par_lhs, Node<Chess> *par_rhs) {
                   return par_lhs->path_cost < par_rhs->path_cost;
                 }))
                ->path_cost;
...
std::ostream &operator<<(std::ostream &par_ostream,
                         const Action<Chess> &par_action) {
...
Action<Chess> ToAction(const std::string &par_orig,
                       const std::string &par_dest) {
...
std::array<Piece, SpaceCount>
Board(const std::array<Piece, SpaceCount> &par_board,
      Player par_player = White) {
  std::array<Piece, SpaceCount> board;
...
State<Chess> Blank() {
  State<Chess> state = {};
  state.board =
      Board({___, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___,
...
State<Chess> Puzzle1() {
  State<Chess> state = {};
  state.board =
      Board({B_R, ___, B_B, ___, ___, B_B, B_K, B_R, B_P, B_P, B_P, ___, ___,
             ___, B_P, B_P, ___, ___, B_N, ___, ___, ___, ___, ___, ___, ___,
             ___, B_Q, B_P, ___, ___, ___, ___, ___, W_B, ___, ___, ___, ___,
             ___, ___, ___, ___, ___, ___, ___, ___, ___, W_P, W_P, W_P, W_P,
             ___, W_P, W_P, W_P, W_R, W_N, W_B, ___, W_K, ___, ___, W_R});
...
State<Chess> Puzzle2() {
  State<Chess> state = {};
  state.board =
      Board({B_R, B_N, ___, B_R, ___, ___, B_K, ___, ___, B_P, B_Q, ___, ___,
             B_P, ___, B_P, B_P, ___, ___, B_P, ___, B_B, B_P, W_B, ___, ___,
             ___, W_P, ___, ___, ___, ___, W_P, ___, ___, ___, W_Q, ___, ___,
             ___, ___, ___, W_P, W_B, ___, ___, ___, ___, ___, ___, ___, ___,
             ___, W_P, W_P, W_P, ___, ___, W_R, ___, W_R, ___, W_K, ___});
...
State<Chess> Puzzle3() {
  State<Chess> state = {};
  state.board =
      Board({___, ___, ___, B_K, ___, ___, ___, ___, ___, ___, ___, ___, ___,
             ___, B_P, ___, ___, ___, ___, W_K, ___, B_P, W_P, B_P, ___, ___,
             ___, W_P, B_R, W_P, ___, ___, B_P, ___, ___, ___, ___, ___, ___,
             ___, W_P, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___, W_Q,
             ___, ___, ___, W_P, ___, B_Q, ___, ___, ___, ___, ___, ___});
  state.king_moved = {true, true};
...
State<Chess> Puzzle4() {
  State<Chess> state = {};
  state.board =
      Board({W_R, W_N, W_B, W_K, ___, W_B, W_N, W_R, W_P, W_P, W_P, ___, ___,
             W_P, W_P, W_P, ___, ___, ___, ___, ___, W_Q, ___, ___, ___, ___,
             ___, W_P, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___,
             ___, ___, ___, ___, ___, ___, B_N, ___, ___, B_P, B_P, B_P, ___,
             B_P, B_P, B_P, B_P, B_R, B_N, B_B, B_K, B_Q, B_B, ___, B_R},
            Black);
  state.king_moved = {false, false};
  state.king_space = {E1, E8};
  state.rookA_moved = {false, false};
...
State<Chess> Puzzle5() {
  State<Chess> state = {};
  state.board =
      Board({___, ___, ___, ___, ___, ___, ___, ___, ___, ___, B_P, ___, B_K,
             ___, ___, B_P, B_P, ___, W_P, ___, ___, ___, ___, ___, ___, B_P,
             ___, ___, ___, ___, ___, ___, ___, ___, ___, B_B, ___, ___, ___,
             ___, ___, ___, ___, ___, W_N, W_P, ___, ___, W_P, ___, ___, W_K,
             ___, ___, W_P, ___, ___, ___, ___, ___, ___, ___, ___, ___});
  state.king_moved = {true, true};
...
State<Chess> Puzzle6() {
  State<Chess> state = {};
  state.board =
      Board({B_R, ___, B_B, B_Q, B_K, ___, ___, B_R, B_P, B_P, B_P, ___, ___,
             B_P, B_P, B_P, ___, ___, ___, ___, ___, ___, ___, ___, ___, ___,
             ___, B_P, W_P, ___, ___, ___, ___, B_B, ___, ___, B_N, ___, ___,
             ___, ___, ___, W_N, W_Q, ___, ___, ___, ___, W_P, W_P, W_P, ___,
             W_B, W_P, W_P, W_P, W_R, ___, W_B, ___, W_K, ___, ___, W_R});
  state.king_moved = {false, false};
  state.king_space = {E1, E8};
  state.rookA_moved = {false, false};
  state.rookH_moved = {false, false};
